...
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/hyperpolymath/zotero-formdb/main/install.ps1" -OutFile "install.ps1"
...
* `formdb-doi` CLI for DOI management
...
* `formdb-publisher` CLI for publisher management
* `formdb-blindspot` CLI for library bias analysis
